            ...
            <div class="row">
                ...
                <div class="col-md-8">
                    ...
                    if ($responseData['success']) {
                        ...
                        <form method="post" action="member_employment.php">
                    ...
                    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                        if (isset($_COOKIE['loginUser'])) {
                            ...
                            if (isset($_POST['jId'])) {
                                $jId = $_POST['jId'];
                                $jId = substr($jId, strpos($jId, "_") + 1);

                                $currentDateTime = date('Y-m-d H:i:s');


                                $url = 'http://3.212.61.233:3000/ApplyJob/';
                                $data = [
                                    "jId" => $jId,
                                    "mId" => $mId,
                                    "application_date" => $currentDateTime
                                ];
                                ...
                                if ($response === false) {
                                    ...
                                    if ($responseData['success']) {
                                        $apiData = $responseData['data'];
                                        echo '<script>alert("Apply Job Success");</script>';
                                    } else {
                                        // API request failed
                                        $errorMsg = $responseData['msg'];
                                        $errorCode = $responseData['code'];
                                        echo '<script>alert("' . $errorMsg . '");</script>';
                                        echo '<script>window.location.href = "member_employment.php";</script>';
                                    }
                                }
